...
function renderExpenses() {
    ...
    // Determine how many expenses to show
    const shouldPaginate = filtered.length > 5;
    const expensesToShow = shouldPaginate && !showAllExpenses ? filtered.slice(0, 5) : filtered;

    list.innerHTML = expensesToShow.map(expense => `
        ...
        const buttonHtml = `
            <div class="show-more-container">
                <button class="show-more-btn" onclick="toggleShowAll()">
                    ${showAllExpenses ? '▲ Show Less' : `▼ Show More (${filtered.length - 5} more)`}
                </button>
            </div>
        `;
...
function switchTab(tab) {
    currentTab = tab;
    showAllExpenses = false; // Reset pagination when switching tabs
